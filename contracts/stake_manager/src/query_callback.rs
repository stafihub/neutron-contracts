...
pub fn write_validator_query_id_to_reply_id(deps: DepsMut, msg: Reply) -> StdResult<Response> {
    let resp: MsgRegisterInterchainQueryResponse = serde_json_wasm::from_slice(
        msg.result
            .into_result()
            .map_err(StdError::generic_err)?
    ...
    deps.api.debug(
        format!(
            "WASMDEBUG: write_validator_query_id_to_reply_id query_id: {:?} msg.id(Reply id): {:?}",
            resp.id, msg.id
        )
        .as_str(),
    );

    KV_QUERY_ID_TO_CALLBACKS.save(deps.storage, resp.id, &QueryKind::Validator)?;
    REPLY_ID_TO_QUERY_ID.save(deps.storage, msg.id, &resp.id)?;
