...
use crate::state::{KV_QUERY_ID_TO_CALLBACKS, OWN_QUERY_ID_TO_ICQ_ID, QueryKind};
...
pub fn write_balance_query_id_to_reply_id(
    deps: DepsMut,
    reply: Reply,
) -> StdResult<Response> {
...
pub fn write_delegation_query_id_to_reply_id(
    deps: DepsMut,
    reply: Reply,
) -> StdResult<Response> {
    let resp: MsgRegisterInterchainQueryResponse = serde_json_wasm::from_slice(
