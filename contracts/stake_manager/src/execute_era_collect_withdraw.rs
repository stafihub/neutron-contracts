...
pub fn execute_era_collect_withdraw(
    ...
    deps.as_ref().api.debug(
        format!(
            "WASMDEBUG: execute_era_collect_withdraw stake_amount: {}, validator_count is {}",
            withdraw_amount, validator_count
        )
        .as_str(),
    );
    ...
    let amount_per_validator = withdraw_amount.div(Uint128::from(validator_count));
    let remainder = withdraw_amount.sub(amount_per_validator.mul(Uint128::new(validator_count)));
